...
#'@param true_fisherPV The Fisher's test p-values for the observations.
...
hclust_tree <- function(pinmat, mat_fdr, true_fisherPV, cell_names, hc_method = "average"){

  ## The distance measure matrix: log10(true_fisherPV)
  #########################################################################
  mat_dist <- matrix(ncol = (1 + sqrt(1 + 8*length(true_fisherPV)))/2,
                     nrow=(1 + sqrt(1 + 8*length(true_fisherPV)))/2, data = 0)
  mat_dist[upper.tri(mat_dist)] <- log10(true_fisherPV)                                          ####?? dist: -log10(fisherPV)
  ...
  return(list(hc = hc, mat_dist = mat_dist))
